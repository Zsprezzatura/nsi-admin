...
function getInitData() {
    ...
    $.ajax({
        ...
        url:'http://' + changeUrl.address + '/Class_Teacher_api?whereFrom=searchTeacherMessage',
        dataType:"json",
        contentType:"application/json;charset=UTF-8",
        async:true,
        success:function (res) {
            console.log(res)
                $('#TeacherName').val(res.data[0].TeacherName)
                $('#TeacherDescription').val(res.data[0].TeacherDescription)
                $('#TeacherCourse').val(res.data[0].TeacherCourse)
                $('#TeacherImage').val(res.data[0].TeacherImage)
...
layui.use(['form', 'laydate'], function(){
    ...
    form.on('submit(demo1)', function(rs){
        ...
        var insertData = {
            'object01':$('#TeacherId').val(),
            'object02': $('#TeacherName').val(),
            'object03': $('#TeacherDescription').val(),
            'object04': $('#TeacherCourse').val(),
            'object05':editor1.txt.html(),
            'object06': $('#TeacherImage').val(),
            'object07': $.cookie('User_TureName')
        }
        $.ajax({
            type: "post",
            async: true,
            data: JSON.stringify(insertData),//提交的参数
            url: 'http://' + changeUrl.address + '/Class_Teacher_api?whereFrom=alter',
            ...
            success: function (msg) {
                console.log(msg)
                if(msg.msg ==1){
                    layer.alert('修改成功,可在概览页面修改，请勿重复提交',{icon:1},function () {
                        //当你在iframe页面关闭自身时
