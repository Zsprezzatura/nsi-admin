    ...
    $.ajax({
        ...
        success:function (res) {
            ...
            layui.use('form',function () {
                var form = layui.form
                console.log(res.data[0].CourseState)
                $("#CourseState input[type=radio][value="+res.data[0].CourseState+"]").attr('checked',true)
                form.render('radio')
            })

            $('#ChannelNumber').val(res.data[0].ChannelNumber)
            $('#ChannelName').val(res.data[0].ChannelName)
            $('#Secretkey').val(res.data[0].Secretkey)
            $('#TeacherId').val(res.data[0].TeacherId)
            $('#CourseSubject').val(res.data[0].CourseSubject)
            $('#CourseName').val(res.data[0].CourseName)

            $('#CourseDescription').val(res.data[0].CourseDescription)
            $('#ClassBegins').val(res.data[0].ClassBegins)
            $('#CoursePraise').val(res.data[0].CoursePraise)
            $('#CoursePrice').val(res.data[0].CoursePrice)

            $('#CourseRelease').val(res.data[0].CourseRelease)
            $('#CoursePeople').val(res.data[0].CoursePeople)
            $('#CoverImage').val(res.data[0].CoverImage)
            $('#CourseImage').val(res.data[0].CourseImage)
   ...
   ,layer = layui.layer
    ...
    form.on('submit(demo1)', function(rs){
        ...
        $.ajax({
            ...
            url: 'http://' + changeUrl.address + '/Class_Course_api?whereFrom=alter_Course',
            dataType: "json",//数据类型为json  
            success: function (msg) {
                console.log(msg)
                if(msg.msg ==1){
                    layer.alert('修改成功，请刷新课程概览页面查看',{icon:1},function () {
                        //当你在iframe页面关闭自身时
